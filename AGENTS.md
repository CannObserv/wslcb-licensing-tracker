...
| `pg_queries.py` | *(PostgreSQL migration — Phase 3)* Async equivalent of `queries.py`. FTS5 replaced by ILIKE on `business_name`, `applicants`, `resolved_endorsements` — marked `# TODO Phase 5: tsvector`. `export_records_cursor()` is `AsyncGenerator[dict, None]`. `outcome_filter_sql` fragments emit `CURRENT_DATE - interval '...'` (PG-native). In-memory TTL caches (`_filter_cache`, `_city_cache`, `_stats_cache`) preserved unchanged. |
