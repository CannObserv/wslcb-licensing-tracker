...
SocratiCode is **pinned** here (`~/.socraticode/pin`, outside the repo): this host has no swap and co-hosts `wslcb-web.service`, and a per-launch `npx socraticode@latest` install was the measured memory peak, not the indexing (#175). Never "fix" the driver back to `npx`; confirm with `mcp-driver.mjs resolve`. The plugin session is pinned too, by `SOCRATICODE_SPEC` in `.claude/settings.json` (#180) — re-pin both to one version. See [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md) **Memory pressure**.
