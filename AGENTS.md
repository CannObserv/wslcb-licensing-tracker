...
| `pg_integrity.py` | *(PostgreSQL migration — Phase 4)* Async port of 3 integrity check functions used by admin dashboard: `check_orphaned_locations()`, `check_unenriched_records()`, `check_endorsement_anomalies()`. Full CLI suite (`fix_orphaned_locations`, `run_all_checks`) is Phase 6. |
