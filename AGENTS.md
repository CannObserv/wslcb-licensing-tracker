...
| `src/wslcb_licensing_tracker/address_validator.py` | Client for address standardization + optional validation API | Two-phase pipeline at `https://address-validator.exe.xyz:8000`. API key in `/etc/wslcb-licensing-tracker/env` (falls back to `<project-root>/env` for local dev). **Phase 1 (always):** `standardize_location()` calls `POST /api/v1/standardize`, writes `std_address_line_1/2`, `std_city`, `std_region`, `std_postal_code`, `std_country`, `std_address_string` (from `standardized`), `validation_status="standardized"`, and sets `address_standardized_at`. **Phase 2 (optional):** `validate_location()` is gated by `ENABLE_ADDRESS_VALIDATION` env var (`"1"`, `"true"`, `"yes"`); if disabled it is a no-op. When enabled, calls `POST /api/v1/validate` (which runs standardize internally — no separate client-side standardize call needed); on confirmed: overlays `std_address_string` (from `validated`), `validation_status`, `dpv_match_code`, `latitude`, `longitude`, sets `address_validated_at`; on `not_confirmed`/`unavailable`: writes `validation_status` + `dpv_match_code` only, leaves `address_validated_at` NULL so `backfill-addresses` retries when flag is turned on. `country` validated as ISO 3166-1 alpha-2 before storage. `backfill_addresses()` queries `address_standardized_at IS NULL OR address_validated_at IS NULL` (skips only locations with both timestamps set). |
...
- Authenticated via `X-API-Key` header; key stored in `/etc/wslcb-licensing-tracker/env` (`ADDRESS_VALIDATOR_API_KEY=...`)
- `/etc/wslcb-licensing-tracker/env` is `640 root:exedev`, gitignored; `_load_api_key()` falls back to `<project-root>/env` for local dev
- Operates on the `locations` table — each unique raw address is processed once and shared across all records that reference it
- **Standardization always runs** (`POST /api/v1/standardize`) — sets `std_*` columns and `address_standardized_at`
- **DPV validation is optional** — set `ENABLE_ADDRESS_VALIDATION=true` in the environment to enable; calls `POST /api/v1/validate` which runs standardize internally (no double-call); sets `address_validated_at` on confirmed addresses
- At scrape time, `validate_record()` skips already-processed locations (checks `address_standardized_at` + flag state)
- Systemd services load the env file via `EnvironmentFile=/etc/wslcb-licensing-tracker/env`
- Backfill: `wslcb backfill-addresses` (processes locations where `address_standardized_at IS NULL OR address_validated_at IS NULL`)
- Refresh: `wslcb refresh-addresses` (re-processes all locations; safe to interrupt)
...
```
Re-standardizes every location against the address-validator API. Safe to interrupt — progress is committed in batches.
If `ENABLE_ADDRESS_VALIDATION=true` is set in the service environment, also re-validates each location via DPV.
