"""Tests for backfill_snapshots.py — two-phase snapshot ingest and repair helpers."""
...
from wslcb_licensing_tracker.backfill_snapshots import (
    _repair_assumptions,
...
async def test_backfill_from_snapshots_completes_without_error(pg_engine):
    ...
    with (
        patch(
            "wslcb_licensing_tracker.backfill_snapshots.snapshot_paths",
            return_value=[SNAPSHOT_FIXTURE],
        ),
        patch("wslcb_licensing_tracker.backfill_snapshots.DATA_DIR", FIXTURES_DIR),
    ):
        # Should not raise.
...
async def test_backfill_from_snapshots_skips_unparseable_snapshot(pg_engine, tmp_path):
    ...
    with (
        ...
        patch("wslcb_licensing_tracker.backfill_snapshots.DATA_DIR", tmp_path),
    ):
